...
package mesquite.treecomp.TreeCompStrictConsensus;
...
import mesquite.treecomp.common.MarkedBipartitionTable;
import mesquite.treecomp.common.PSWTree;

public class TreeCompStrictConsensus extends Consenser {
	...
	public String getName() {
		return "TreeComp Strict Consensus";
	}
	...
	public String getAuthors() {
		return "Jeff Klingner, The University of Texas at Austin, with Silvio Neris, City University of New York";
	}
	...
	public Tree consense(Trees list) {
		...
		// The first step is to get PSW (postorder sequence with weights)
		// representations for all of the trees
		Tree tempTree;
		PSWTree tempPSW;
		...
		/*
		 * mark edges in the bipartition table for removal if they are not found
		 * in all other trees
		 */
		...
		if (!allTreesHaveTwoChildRoots) {
			answer.collapseBranch(answer.nextAroundUR(answer.getRoot(), answer
					.nodeOfTaxonNumber(0)), false);
		}
		// System.out.println("after collapsing pseudo-root: " +
		// answer.writeTree(MesquiteTree.BY_NAMES));
	...
	private boolean checkChildrenOfRoot(Tree t) {
		return (t.daughtersOfNode(t.getRoot()).length == 2)
				&& (t.firstDaughterOfNode(t.getRoot()) == t
						.nodeOfTaxonNumber(0));
	}
	...
	private void checkAgainstTable(PSWTree tree, MarkedBipartitionTable table) {
		...
		// rememberedTrees[tree2Index].getName() + " against table for " +
		// rememberedTrees[tree1Index].getName());

		for (tree.prepareForEnumeration(), vw = tree.nextVertex(); vw != null; vw = tree
				.nextVertex()) {
			lrnw = new int[4];
			if (vw[1] == 0) { /* leaf vertex */
				...
				lrnw[0] = tree.getN() + 1; /*
											 * we want the smallest leaf among
											 * this vertex's children with
											 * min(). This is bigger than any
											 * leaf
											 */
				lrnw[1] = 0; /*
							 * likewise, smaller than ever leaf among my
							 * children
							 */
				lrnw[2] = 0; /* start with a leaf count of zero */
				lrnw[3] = 1; /*
							 * will be total weight; start with one for this
							 * vertex
							 */

				w = vw[1]; /* total weight below this vertex */
				while (w > 0) { /*
								 * while there are still children unaccounted
								 * for
								 */
					poppedLRNW = stack.pop();
					lrnw[0] = Math.min(lrnw[0], poppedLRNW[0]); /*
																 * smallest #ed
																 * leaf below me
																 */
					lrnw[1] = Math.max(lrnw[1], poppedLRNW[1]); /*
																 * largest #ed
																 * leaf below me
																 */
					lrnw[2] = lrnw[2] + poppedLRNW[2]; /*
														 * total up the leaves
														 * below me
														 */
					lrnw[3] = lrnw[3] + poppedLRNW[3]; /*
														 * total up the weight
														 * below me
														 */
					w = w - poppedLRNW[3];
				...
				/*
				 * Now, for the subtree below this interior vertex, we have the
				 * total number of leaves (N), and the smallest (L) and largest
				 * (R) #ed leaves (according to the encoding used for tree1's
				 * bipartition table). If the leaves below this vertex form a
				 * contiguous sequence (i.e. if N=R-L+1) then we have a
				 * candidate bipartiton that could be in tree 1. If this is the
				 * case, we check tree1's bipartition talbe for it and keep
				 * score accordingly.
				 */
				if (!( // two of the clusters we get in the traversal are not
						// bipartitions
				(lrnw[0] == 1 && lrnw[1] == tree.getN()) // cluster of
															// everything but
															// taxon zero - not
															// a bipartition
				|| (lrnw[0] == 1 && lrnw[1] == tree.getN() - 1) // cluster that
																// includes the
																// whole tree -
																// not a
																// bipartition
				)) {
					if (lrnw[2] == lrnw[1] - lrnw[0] + 1) {/*
															 * if (N = R - L +
															 * 1)
															 */
						table.touchBipartition(lrnw[0], lrnw[1]); // if
																	// bipartition
																	// is in the
																	// table, it
																	// is marked
	...
	private void contractAgainstTable(MesquiteTree answer, PSWTree tree,
			MarkedBipartitionTable table) {
		stack.empty();
		int[] lrnw, poppedLRNW; /* see description below at initialization */
		int[] vw; /* the current vertex,weight pair */
		int w; /* total weight unaccounted for beneath the current vertex */
				...
				|| (lrnw[0] == 1 && lrnw[1] == tree.getN() - 1) // cluster that
					...
					if (lrnw[2] == lrnw[1] - lrnw[0] + 1) {/*
						...
						if (!table.containsMarkedBipartition(lrnw[0], lrnw[1])) {
							// this bipartition was not it all the trees.
							// contract its associated edge
	...
	/*
	 * rearranges branches (by spinning nodes only) so that nodes in every
	 * subtree are sorted by node number
	 */
	...
	private static int recursiveSorter(Tree t, int currentNode) {
		if (t.nodeIsTerminal(currentNode)) {
			...
			int temp;
			for (int d = t.firstDaughterOfNode(currentNode); t.nodeExists(d); d = t
					.nextSisterOfNode(d)) {
				daughterScores[daughterCounter++] = recursiveSorter(t, d);
			}
			for (int i = 1; i < numDaughters; i++) {
				for (int j = i - 1; j >= 0
						&& daughterScores[j] > daughterScores[j + 1]; j--) {
					((MesquiteTree) t).interchangeBranches(t
							.indexedDaughterOfNode(currentNode, j), t
							.indexedDaughterOfNode(currentNode, j + 1), false);
