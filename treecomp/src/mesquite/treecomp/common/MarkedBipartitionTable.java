...
public class MarkedBipartitionTable {
	...
	private int numberOfBipartitions; /*
									 * in the sense of the Day paper, after
									 * pseudo-rooting
									 */

	public MarkedBipartitionTable(PSWTree tempPSW) {
		tableSize = tempPSW.getN() + 1;
		...
		int L, R, leafcode;
		int[] vw;
		tempPSW.prepareForEnumeration();
		leafcode = 0;
		R = 0;
		vw = tempPSW.nextVertex();

		while (vw != null) {
			if (vw[1] == 0) { /* v is a leaf */
				leafcode++;
				encoding[vw[0]] = leafcode;
				// System.out.println("encoding[ " + vw[0] + " ] = " + leafcode
				// + " = R");
				R = leafcode;
				vw = tempPSW.nextVertex();
			} else { /* v is an internal vertex */
				L = encoding[tempPSW.leftmostLeaf()];
				// System.out.println("encoding[ " + t.leftmostLeaf() + " ] = "
				// + L + " = L");
				vw = tempPSW.nextVertex();
				if (!( // two of the clusters we get in the traversal are not
						// bipartitions and shouldn't go in the table
				(L == 1 && R == tempPSW.getN()) // cluster of everything but
												// taxon zero - not a
												// bipartition
				|| (L == 1 && R == tempPSW.getN() - 1) // cluster that includes
														// the whole tree - not
														// a bipartition
				)) {
