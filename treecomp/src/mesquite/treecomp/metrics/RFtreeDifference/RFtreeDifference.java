...
/**
 * This is a reimplementation of RFtreeDifference, created by Wayne Maddison. I
 * wrote it because the existing module was too slow for the the large number of
 * calls required by TreeSetVisualization. I have tried to make this as fast as
 * I could, sacrificing unnecesary abstractions and coherence of style with the
 * rest of Mesqutie for the sake of space and time efficiency.
 * 
...
public class RFtreeDifference extends NumberFor2Trees {
	...
	public String getName() {
		return "Robinson-Foulds Tree Difference";
	}
	...
	public String getAuthors() {
		return "Jeff Klingner, The University of Texas at Austin";
	}
	 ...
	 * ......................................
	 */
	public void calculateNumber(Tree tree1, Tree tree2, MesquiteNumber result,
		...
		if (result == null) {
			System.out
					.println("You passed an uninitialized result holder to calculateNumber().");
			return;
		}

		if (tree1.getTaxa() != tree2.getTaxa()) {
			System.out
					.println("RFtreeDifference only works for trees over the same Taxa.");
			return;
		}

		/*
		 * First, we check to see if we have seen either of the trees before. If
		 * we haven't we compute a PSW tree and a bipartition table for them For
		 * the calculation to come, we need a table for tree1 and a PSW for
		 * tree2
		 */
		...
		if (!PSWs.containsKey(tree1)) {
			...
			table = tempTable;
		} else { // we have already seen this tree; retrieve its table from
					// memory
		...
		/*
		 * To compute RF difference, we can't compare bipartition tables
		 * directly, because the two tables may have been (and probably were)
		 * constructed using two different leaf relabeling functions. Instead we
		 * traverse one tree (using it's PSW representation) and check for the
		 * bipartitions we find in the other tree's bipartition table. This is
		 * still O(n).
		 */
		...
		// System.out.println("Traversing " + tree2.getName() +
		// " against table for " + tree1.getName());
			...
			if (vw[1] == 0) { /* leaf vertex */
				...
				|| (lrnw[0] == 1 && lrnw[1] == tree.getN() - 1) // cluster that
					...
					if ((lrnw[2] == lrnw[1] - lrnw[0] + 1) /*
						...
						misses++;
						// System.out.println("Miss: <" + lrnw[0] + "," +
						// lrnw[1] + ">");
					}
				}
			}
		...
		// table.getNumBipartitions() + " bipartitions in tabled tree");
