 ...
 * @author Damian
 */
public class MatchingSplitOpt3Metric extends BaseMetric implements Metric {
...
*/
        leafT1toLeafT2=matchLeavsT1toT2(t1,t2);
...
/*
  ...
  */
        ...
        System.out.println("Execution of MS opt3 time was "+(end-start)+" ms.");
        System.out.println("Execution of Lap for MS opt3 time was "+(end-lapStart)+" ms.");
        return metric;
...
int[] matchLeavsT1toT2(Tree t1,Tree t2){
    int size=t1.getExternalNodeCount();
    int[] t1ToT2=new int[size];
    int i,j=-1;
    Node n1,n2;
    String n1Id;
    ArrayList<Node> n2Nodes=new ArrayList<Node>(size);
    for(i=0;i<size;i++){
        n2=t2.getExternalNode(i);
        n2Nodes.add(n2);
    }
     for(i=0;i<size;i++){
         n1=t1.getExternalNode(i);
         n1Id=n1.getIdentifier().getName();
         for(Node n:n2Nodes){
             if(n1Id.equals(n.getIdentifier().getName())){
                 j=n.getNumber();
                 break;
             }
         }
